...
then
	...
	then
		...
		Out=$PATH_Output/$FILE_base
		mkdir -p $Out/Polished_genomes_POLCA
		echo "Polishing is started, time: $(date +"%T")"
		{ time bash Polishing_Polca.sh > $Out/Polished_genomes_POLCA/Polishing.log 2> $Out/Polished_genomes_POLCA/Polishing.elog ; } 2> $Out/Polished_genomes_POLCA/Polishing.time.txt 
		conda deactivate
	fi
	if [ $NextPol = 1 ]
	then
...
then
	...
	{ time bash BUSCO.sh $Out/Polished_genomes/Polished_genomes_final polished.fasta > $Out/BUSCO/BUSCO.log 2> $Out/BUSCO/BUSCO.elog ; } 2> $Out/BUSCO/BUSCO.time.txt
...
then
	...
	{ time bash Quast.sh $Out/Polished_genomes/Polished_genomes_final polished.fasta > $Out/Quast/Quast.log 2> $Out/Quast/Quast.elog ; } 2> $Out/Quast/Quast.time.txt
...
then
	...
	{ time bash KAT4.sh > $Out/KAT_${FILE_base}/KAT_read.log 2> $Out/KAT_${FILE_base}/KAT_read.elog ; } 2> $Out/KAT_${FILE_base}/KAT_read.time.txt # read stat
	echo -e "\nKAT-read is finished, time:\n" `cat $Out/KAT_${FILE_base}/KAT_read.time.txt`

	echo "KAT on assemblies is started, time: $(date +"%T")"
	{ time bash KAT_assembly5.sh > $Out/KAT_${FILE_base}/KAT_assembly.log 2> $Out/KAT_${FILE_base}/KAT_assembly.elog ; } 2> $Out/KAT_${FILE_base}/KAT_assembly.time.txt
...
then
	...
	then
		...
		mkdir -p $Out/Quast_ignore/polished
		mkdir -p $Out/Quast_ignore/polished_meta
	...
	source activate base
	time multiqc -o $Out/MultiQC --ignore \\""$ignore"\\" --ignore "$Out/Polished_genomes/Polishing_files" -p -d -dd 1 $Out/FastQC_$FILE_base $Out/BUSCO/polished/Summary_files $Out/Polished_genomes/Polished_genomes_stat $Out/Quast/polished/Reports/$ReportWhat $Out/Trimmomatic_$FILE_base $Out/KAT_${FILE_base}
	# place quast repot in separate folders
	conda deactivate
fi