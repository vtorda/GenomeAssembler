...
then
	...
	#mkdir -p $out1
	megahit --no-mercy --min-count 3 --k-list ${bestk},31,51,71,91,99 -t $Threads -1 ${INPUT}/*Pair_1*.fastq.gz -2 ${INPUT}/*Pair_2*.fastq.gz \
...
then
	...
	#mkdir -p $out1
	megahit --min-count 2 --k-list ${bestk},21,31,41,51,61,71,81,91,99 -t $Threads -1 ${INPUT}/*Pair_1*.fastq.gz -2 ${INPUT}/*Pair_2*.fastq.gz \
