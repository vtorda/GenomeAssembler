...
until bash KAT.sh # read stat
do
	echo "Restart KAT"
	sleep 2
done
until bash KAT_assembly.sh ${Out}/ABySS_${FILE_base}/Trimmomatic_hard ${Out}/Trimmomatic_${FILE_base}/Trimmomatic_hard # read stat
do
	echo "Restart KAT"
	sleep 2
done
until bash KAT_assembly.sh ${Out}/ABySS_${FILE_base}/Lighter_RHW620_downsB ${Out}/Lighter_${FILE_base} # read stat
do
	echo "Restart KAT"
	sleep 2
done
