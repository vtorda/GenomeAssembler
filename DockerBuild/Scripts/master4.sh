...
lighter -r $INPUT/${FILE_base}_AdaptRemovePair_1.fastq.gz -r $INPUT/${FILE_base}_AdaptRemovePair_2.fastq.gz \
			-k $kmer_length $genome_size $c -t $Threads -od $OutputDir > $OutputDir/lighter.log 2> $OutputDir/lighter.log2
...
mkdir $OutputDir
FILES=$Out/Lighter_${FILE_base}/${FILE_base}*fq.gz
for f in ${FILES}
	do
		fastqc $f -o $OutputDir -t $Threads
		wait
...
for k in ${bestk} 33 50 70 90 110 120
do
