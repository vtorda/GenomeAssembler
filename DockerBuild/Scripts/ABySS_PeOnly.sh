...
for kc in ${kc_list}; do
	for k in ${bestk} ${K_list}; do
		...
		echo abyss-pe -C $OutputDir/kmer_${k}_kc_${kc} in="${INPUT}/*Pair_1*.fastq.gz ${INPUT}/*Pair_2*.fastq.gz" name=ABySS_${Folder}_${FILE_base}_kmer${k}kc${kc} v=-v k=$k kc=$kc B=$Abyss_mem j=$Threads
		abyss-pe -C $OutputDir/kmer_${k}_kc_${kc} in="${INPUT}/*Pair_1*.fastq.gz ${INPUT}/*Pair_2*.fastq.gz" \
						name=ABySS_${Folder}_${FILE_base}_kmer${k}kc${kc} v=-v k=$k kc=$kc B=$Abyss_mem j=$Threads
	done
